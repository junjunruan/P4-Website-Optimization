 ...
 */
 var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    ...
    switch(size) {
      case "1":
        // move this querySelector outside of switch
   ...
   * @param {number} elem - pizza element.
   * @param {number} size - pizza size.
   * @return {number} dx - size difference to change a pizza element.
   */
  ...
  */
  function changePizzaSizes(size) {
    // Optimization: create an array variable that has a reference to all of the pizzas with class name "randomPizzaContainer"
    var randomPizzaContainer = document.getElementsByClassName("randomPizzaContainer");

    //Optimization: create a variable to store the length of randomPizzaContainer
    var randomPizzaContainerLen = randomPizzaContainer.length;

    //Optimization: move dx and newwidth outside of loop, as they are only related to size
    var dx = determineDx(randomPizzaContainer[0], size);
    var newwidth = (randomPizzaContainer[0].offsetWidth + dx) + 'px';

    for (var i = 0; i < randomPizzaContainerLen; i++) {
...
function updatePositions() {
  ...
  //Optimization: getElementsByClassName is faster than querySelectorAll
  // var items = document.querySelectorAll('.mover');
  var items = document.getElementsByClassName('mover');

  //Optimization: scrollTop is a constant value, move it outside of loop
  var scrollTopValue = document.body.scrollTop / 1250;

  //Optimization: calculate phase and store it into an array outside of loop
  var phase = [];
  for (var j = 0; j < 5; j++) {
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //Optimization: define elem out side of for loop
  var elem;
  //Optimization: create an array variable that has a reference to all of the pizzas with id "movingPizzas1"
  var movingPizzas1 = document.getElementById("movingPizzas1");
  //Optimization: change the number of  moving pizzas created in the background from 200 to 50, as 200 pizzas are too much.
  for (var i = 0; i < 50; i++) {
